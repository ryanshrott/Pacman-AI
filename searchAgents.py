...
#  print 'Type'
  def ucs(state, problem):
      ...
      # We perform UCS on a relaxed problem with all walls removed 
      relaxed_problem = deepcopy(problem)
      relaxed_problem.walls = Grid(w.width, w.height, initialValue=False)
      # add the boundary walls
      # bottom and top row
      for i in range(w.width):
          relaxed_problem.walls[i][0] = True
          relaxed_problem.walls[i][w.height-1] = True
      for i in range(w.height):
          relaxed_problem.walls[0][i] = True
          relaxed_problem.walls[w.width-1][i] = True 
          
      #print relaxed_problem.walls
      sol = uniformCostSearch(relaxed_problem)
      
      return problem.getCostOfActions(sol)
  
  def manhattan(x, y):
      return (abs(x[0]-y[0]) + abs(x[1]-y[1])) 
  
  def furthest_food(problem, state):
      #print problem._expanded
      position, foodGrid = state
      food_pos = foodGrid.asList()
      for x in food_pos:
          if (x,position) not in problem.heuristicInfo.keys() and (position,x) not in problem.heuristicInfo.keys():
              d = mazeDistance(position, x, problem.startingGameState)
              problem.heuristicInfo[(position,x)] = d
              problem.heuristicInfo[(x,position)] = d
      food_dict = {x:problem.heuristicInfo[(position, x)] for x in food_pos}
      if food_dict:
          furthest = max(food_dict, key=food_dict.get)
          return food_dict[furthest]
      else:
          return 0 # no more food 
  def travelling_salesman_dynamic_programming(problem, state):
      position, foodGrid = state
      print problem._expanded
      # help karp algorithm      
      cities = foodGrid.asList()
      cities.insert(0, position)
      #print cities
      if len(cities) > 1:
          for x in cities:
              for y in cities:
                  if (x,y) not in problem.heuristicInfo.keys() and (y,x) not in problem.heuristicInfo.keys():
                      print 'Do computation!'
                      if x != y:
                          d = mazeDistance(x, y, problem.startingGameState)
                      else:
                          d = 0
                      problem.heuristicInfo[(x,y)] = d
                      problem.heuristicInfo[(y,x)] = d # By symmetry 
          dist_matrix = [[problem.heuristicInfo[(x,y)] for y in cities] for x in cities]      
          #print(dist_matrix)
  ...
  return furthest_food(problem, state)
...
class AnyFoodSearchProblem(PositionSearchProblem):
  ...
  def isGoalState(self, state):
    ...
    "*** YOUR CODE HERE ***"
    # get all the states with food 
    food_locations = self.food.asList()
    # if we are sitting on food now, we are done 
